$(document).ready(function(){
	...
	function validarFotos()
	{
		...
		if(foto.value.trim() == '')
		{
			...
			target.parentNode.getElementsByTagName('small')[0].innerHTML = 'Deve ser selecionado no minimo uma foto';
			target.classList.remove('valido');
			target.setAttribute('class', 'invalido');
			//console.log('Deve ser selecionado no minimo uma foto');
			return false;
		}
		else if(!validarExt())
		{
		...
		else if(foto.files[0].size > 700000)//Não pode ser maior que 7mbs
		{
			var target = document.getElementById('fotos');
