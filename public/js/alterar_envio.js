$(document).ready(function(){
	...
	function validarFotos()
	{
		...
		if(foto.value.trim() != '')//Se for enviado uma imagem
		{
			...
			else if(foto.files[0].size > 7000000)
			{
				var target = document.getElementById('fotos');
				target.parentNode.getElementsByTagName('small')[0].innerHTML = 'Foto não pode ser maior que 7mbs';
				target.classList.remove('valido');
				target.setAttribute('class', 'invalido');
			}
			else
	...
	function deletarFoto(id_receita,id_foto)
	{
		...
		$.ajax({
			type: "POST",
			url: document.location.origin + '/receitasOn/public/alterar_envio' + '/deletarFoto',
